 ...
 */
// 每一个增强器会调用该方法并传入reducer and preloadedState
...
export default function createStore<S, A extends Action, Ext = {}, StateExt = never>(
  ...
  if (typeof preloadedState === 'function' && typeof enhancer === 'undefined') {
    // 此时  preloadedState  则是增强器
    enhancer = preloadedState as StoreEnhancer<Ext, StateExt>
    preloadedState = undefined
  }
  // 当在增强器中调用createStore时 当前的 enhancer 始终为空
  if (typeof enhancer !== 'undefined') {
    if (typeof enhancer !== 'function') {
      // 增强器类型报错
      throw new Error(`Expected the enhancer to be a function. Instead, received: '${kindOf(enhancer)}'`)
    }
    // return 增强器类型合法，会先开始执行增强器 也就是 applyMiddleware方法中的每一个方法
    // 每一个reducer中会
  ...
  // 当在增强器中使用createStore时，reducer 一定为 function,否则报错
  if (typeof reducer !== 'function') {
    throw new Error(`Expected the root reducer to be a function. Instead, received: '${kindOf(reducer)}'`)
  }
  // 存在增强器时， currentReducer  === combineReducers函数的返回值函数
  // 否则 currentReducer 则为 reducer
  let currentReducer = reducer
  // 存在增强器时  preloadedState === undefined
